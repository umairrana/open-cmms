...
public class GUIPreventiveMaintenanceLog extends MPanel {
    ...
    private void saveWorkOrder() throws Exception{
        ...
        for(int i=0;i<rows;i++){
            ...
            reschedulePlan(
                    ...
                    planningAsset.AVERAGE_METER_PER_DAY
            );
            /* 
            modifyPlan(
                    pmle,
    ...
    private void reschedulePlan(PreventiveMaintenanceLogEntry pmle, 
                               String pm_id, 
                               String modified_date, 
                               String modified_meter,
                               int avg) throws Exception{
                               
        AssetPM assetPM = new AssetPM(txtAsset.getInputText(),pm_id,connection); 
        if(assetPM.isTimeBased()){
            assetPM.deletePlanFrom(modified_date);
            assetPM.planFrom(pmle.done_date,assetPM.PLAN_END_DATE,avg);
        }
        else{
            assetPM.deletePlanFrom(Integer.parseInt(pmle.done_meter));
            assetPM.planMeterBasedFromMeter(pmle.done_date,Integer.parseInt(pmle.done_meter),assetPM.PLAN_END_DATE,avg);
        }
        
        /*
        int days_diff = dateDifference(pmle.done_date,modified_date);
        System.out.println("days_diff:" + days_diff);
        
        if(days_diff != 0){
            assetPM.rescheduleFrom(modified_date,days_diff,0);
            System.out.println("days_diff != 0");
        }
        
        int meter_diff = Integer.parseInt(pmle.done_meter) - Integer.parseInt(modified_meter);
        System.out.println("meter_diff:" + meter_diff);
        
        if( (meter_diff != 0) && 
            (meter_diff != (Integer.parseInt(pmle.done_meter))) ){
            //Timebased PMs have 0 Modified Meter values.
            assetPM.rescheduleMeterBasedMeterOnly(Integer.parseInt(modified_meter),meter_diff);
            //assetPM.rescheduleFrom(modified_date,0,meter_diff);
            //System.out.println("meter_diff != 0");
        }
        
        String newPlanEndDate = assetPM.getLastScheduledDate();
        //System.out.println("newPlanEndDate:"+newPlanEndDate);
        //System.out.println("assetPM.PLAN_END_DATE:"+assetPM.PLAN_END_DATE);
        
        int endDifference = dateDifference(newPlanEndDate,assetPM.PLAN_END_DATE);
        ...
        if(endDifference > 0){
            ...
            assetPM.planFrom(newPlanEndDate,assetPM.PLAN_END_DATE,avg);
            //Extend
        }
        */
    }
    ...
    private void modifyPlan(PreventiveMaintenanceLogEntry pmle, String pm_id, String modified_date, String modified_meter) throws Exception{
        ...
        int days_diff = dateDifference(pmle.done_date,assetPM.getLastDoneDate());
        ...
        if(days_diff > 0 || meter_diff > 0){
            ...
            int schedule_date_diff = dateDifference(pmle.done_date,modified_date);
...
//            tblJobCompletion.populate(WORK_ORDER.asset_id);
    ...
    private void undo_job() throws Exception {
        ...
        for(int i=0;i<rows;i++){
            ...
            pmle.done_date = "0000-00-00 00:00:00";
            tblWorkOrder.setValueAt(pmle.done_date,i,6);
            pmle.done_meter = "0";
            tblWorkOrder.setValueAt(pmle.done_meter,i,7);
