...
import com.matrix.focus.log.PlannedPartUtilisationDialog;
...
public class GUIPreventiveMaintenanceWorkOrder extends MPanel {
    ...
    private JCheckBox chkRegularVisit = new JCheckBox();
    ...
    private void setMode(String mode){
        if(mode.equals("NEW_WORKORDER")){
            ...
            chkRegularVisit.setEnabled(true);
        }
            ...
            btnHistory.setEnabled(false);
            chkRegularVisit.setEnabled(false);
            chkRegularVisit.setSelected(false);
            clearAll();
        }
    ...
    private void jbInit() throws Exception {
        ...
        txtWODate.setBounds(new Rectangle(250, 150, 180, 20));
        txtWODate.setCaption("Job Info:");
        txtWODate.setTxtWidth(120);
        txtWODate.setEditable(false);
        txtWODate.setLblWidth(60);
        jScrollPane2.setBounds(new Rectangle(15, 175, 420, 45));
        jLabel1.setText("Job Description");
        jLabel1.setBounds(new Rectangle(25, 155, 150, 10));
        ...
        btnHistory.setBounds(new Rectangle(250, 125, 75, 20));
        btnHistory.setEnabled(false);
        ...
        chkRegularVisit.setText("Regular Visit");
        chkRegularVisit.setBounds(new Rectangle(340, 25, 95, 20));
        ...
        jPanel3.add(chkRegularVisit, null);
        jPanel3.add(btnHistory, null);
    ...
    private void btnSave_actionPerformed(ActionEvent e){
        ...
        if(txtPromisedDate.getInputText().isEmpty() || txtPromisedDate.getInputText().equals("0000-00-00") ){
            messageBar.setMessage("You have not selected a Promised Date","WARN");
            return;
        }
        
        if(chkRegularVisit.isSelected()){
            int rows = tblWorkOrder.getRowCount();
            if(rows == 0){
                messageBar.setMessage("Regular visit jobs must have at least one PM-0 scheduled task","ERROR");
                return;
            }
            else{
                boolean exist = false;
                for(int i=0; i < rows; i++){
                //JOptionPane.showMessageDialog(null,chkRegularVisit.isSelected()+" "+tblWorkOrder.getValueAt(i,0).toString());
                    if(tblWorkOrder.getValueAt(i,2).toString().equals("PM-0")){
                        exist = true;
                    }
                }
            ...
            if(exist){
                messageBar.setMessage("Jobs that have at least one PM-0 scheduled task must be a regular visit job","ERROR");
                return; 
            }
        }
        ...
        if(txtAsset.getInputText().isEmpty()){ 
            ...
            String reason = "";
            if( !promisedDate.isEmpty() && (!promisedDate.equals(txtPromisedDate.getInputText()))){
           
                while(true){
                    reason = MessageBar.showInputDialog(frame,"Promised date has changed. Please input reason","Reason");
                    if(reason != null){
                        if(!reason.isEmpty()){
                           break;
                        }
                        ...
                        messageBar.setMessage("Job not saved","WARN");
                        return;
                    }
               }
            }
            ...
            try{
                ...
                WORK_ORDER.regularVisit = (chkRegularVisit.isSelected()? "true":"false");
                WORK_ORDER.update();
                
                if(!promisedDate.equals(txtPromisedDate.getInputText())){
                    PreparedStatement pS = connection.prepareStatement("UPDATE promised_date_change_log SET valid= ?");
                    pS.setString(1,"false");
                    pS.executeUpdate();
                    
                    PreparedStatement p = connection.prepareStatement("INSERT INTO promised_date_change_log (PM_Work_Order_ID,Promised_Date,Reason,Valid) VALUES(?,?,?,?)");
                    p.setString(1,mtxtWorkorder.getInputText());
                    p.setString(2,txtPromisedDate.getInputText());
                ...
                setMode("SEARCH");
                promisedDate = txtPromisedDate.getInputText();
    ...
    public void setWorkOrder(String workorder_id){
        ...
        try{
            ...
            promisedDate = WORK_ORDER.promised_date;
            ...
            chkRegularVisit.setSelected(WORK_ORDER.regularVisit.equals("true"));
            setMode("SEARCH");
        } 
        catch(Exception er) {
            messageBar.setMessage(er.getMessage(),"ERROR");
    ...
    private void btnHistory_actionPerformed(ActionEvent e) {
        JScrollPane jScrollPane1 = new JScrollPane();
        HistoryTable hT =  new HistoryTable();
        try {
            hT.populate(mtxtWorkorder.getInputText());
        } catch (Exception f) {
            f.printStackTrace();
        }
        jScrollPane1.getViewport().add(hT);
        jScrollPane1.setBounds(new Rectangle(5, 5, 335, 115));
        
        JDialog jD = new JDialog(frame,"History",true);
        jD.getContentPane().setLayout( null );
        jD.setSize(new Dimension(350, 187));
        jD.getContentPane().add(jScrollPane1, null);
    ...
    private class HistoryTable extends JTable {
         private HistoryTableModel myModel = null;
         ...
         public HistoryTable(){
             ...
             this.getColumnModel().getColumn(1).setCellRenderer(new ButtonCellRenderer());
             ...
             final TaskRemarksDialog dlgPMRemarks = new TaskRemarksDialog(frame);
             dlgPMRemarks.setLocationRelativeTo(frame);
             
             this.addMouseListener(new MouseAdapter(){
                  public void mouseClicked(MouseEvent me){
                      if(me.getButton()==1){
                        int row = HistoryTable.this.getSelectedRow();
                          if(row!=-1){
                             if(HistoryTable.this.getSelectedColumn()==1){                                              
                                  dlgPMRemarks.setText(HistoryTable.this.getValueAt(row,1).toString());
         ...
         public void populate(String workorder_id) throws Exception{
            String sql = "SELECT * FROM promised_date_change_log WHERE PM_Work_Order_ID= ?";
            ...
            stmt.setString(1,workorder_id);
             ...
             while(rec.next()){
                 this.addRow();
                 cnt = rec.getRow()-1;
                 this.setValueAt(rec.getString("Promised_Date").substring(0,10),cnt,0);
                 this.setValueAt(rec.getString("Reason"),cnt,1);
                 this.setValueAt(rec.getString("Valid"),cnt,2);
