...
public class AssetPM{
    ...
    public boolean  HAS_DONE_WORK_ORDERS;
    public String   LAST_DONE_DATE;
    ...
    public AssetPM(String asset_id, String pm_id, Connection connection) throws Exception{
        ...
        this.PLAN_END_DATE = getPlanEndDate();
        this.HAS_DONE_WORK_ORDERS = hasDoneWorkOrders();
        this.LAST_DONE_DATE = ( this.HAS_DONE_WORK_ORDERS ? getLastDoneDate() : "0000-00-00");
    }
    ...
    private boolean hasDoneWorkOrders() throws Exception{
        String sql = "SELECT    Scheduled_ID " +
                     "FROM      scheduled_preventive_maintenance " +
                     "WHERE     Preventive_Maintenance_ID = ? " +
                               "AND " +
    ...
    public void planMeterBasedFromMeter(String starting_date, int starting_meter, String ending_date, int average_meter_per_day) throws Exception{           
        PreparedStatement stmt = this.connection.prepareStatement("CALL planMeterBasedFrom(?,?,?,?,?,?)");
        stmt.setString(1,ASSET_ID);
        stmt.setString(2,PM_ID);
        stmt.setString(3,starting_date);
        stmt.setInt(4,starting_meter);
        stmt.setString(5,ending_date);
        stmt.setInt(6,average_meter_per_day);
    ...
    public void rescheduleMeterBasedMeterOnly(int from_meter,int meter_diff) throws Exception{
        PreparedStatement stmt = this.connection.prepareStatement("CALL rescheduleMeter(?,?,?,?)");
        stmt.setString(1,ASSET_ID);
        stmt.setString(2,PM_ID);
        stmt.setInt(3,from_meter);
        stmt.setInt(4,meter_diff);
    ...
    public void deletePlanFrom(int meter) throws Exception{
        String sql = "DELETE FROM scheduled_preventive_maintenance WHERE Modified_Meter > ? AND Preventive_Maintenance_ID =? AND Asset_ID =?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setInt(1,meter);
        stmt.setString(2,PM_ID);
        stmt.setString(3,ASSET_ID);
    ...
    private String getPlanEndDate()  throws Exception{
        
        PreparedStatement stmt_1 = connection.prepareStatement("SELECT PlanEndMeter, AverageMeterPerDay, Planning_Basis FROM asset WHERE asset_id=?");
        stmt_1.setString(1,ASSET_ID);
        ResultSet rec_1 = stmt_1.executeQuery();
        rec_1.first();
        String basis = rec_1.getString("Planning_Basis");
        
        if(basis.equals("Date")){
            PreparedStatement pS=connection.prepareStatement("SELECT IF(PlanEndDate='0000-00-00','0000-00-00',PlanEndDate) AS PlanEndDate FROM asset WHERE asset_id=?");
            pS.setString(1,ASSET_ID);
            ResultSet rS = pS.executeQuery();
            rS.first();
            return rS.getString("PlanEndDate");
        }
        else{
            int machinePlanEndMeter = rec_1.getInt("PlanEndMeter");
            int machineAverage = rec_1.getInt("AverageMeterPerDay");
            ...
            int planning_days = (machinePlanEndMeter - startMeter) / machineAverage;
