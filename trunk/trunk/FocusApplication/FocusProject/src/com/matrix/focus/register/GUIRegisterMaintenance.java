...
public class GUIRegisterMaintenance extends MPanel{
                    ...
                    try{
                        ...
                        try{
                            if(mpm.isPlanned()){
                                ...
                                else if(mpm.hasDoneWorkOrders()){
                                    mpm.deletePlanFrom(mpm.getLastDoneDate());
                                    mpm.stopPlanning();
                                    connection.commit();
                ...
                edited = new PreventiveMaintenanceRegisterEntry(connection);
                    ...
                    if(edited.save()){
                        ...
                        if(machine.hasPlannedPMs()){
                            mpm.planFrom(mpm.PLAN_START_DATE,mpm.getPlanEndDate(),machine.AVERAGE_METER_PER_DAY);
                        }
                    }
    ...
    private void modifyPlan(PlanningAsset machine, 
        ...
        if(!original.basis.equals(edited.basis)){
            /**Planning basis has been changed*/
            
            if(mpm.isPlanned()){
                if(mpm.hasPendingWorkOrders()){
                    throw new Exception("PM " + mpm.PM_ID + " is in one or more pending Jobs. You can not change its' Basis without removing them from those Jobs.");
                }
                else if(mpm.hasDoneWorkOrders()){
                    mpm.deletePlanFrom(mpm.getLastDoneDate());
                    mpm.planFrom(mpm.PLAN_START_DATE,mpm.getPlanEndDate(),machine.AVERAGE_METER_PER_DAY);
        ...
        if(!original.cycle_time.equals(edited.cycle_time)){
            //Cycle time has been changed;
            if(mpm.isPlanned()){
                int days_difference = (Utilities.getDays(edited.cycle_time) - Utilities.getDays(original.cycle_time));
                if(mpm.hasDoneWorkOrders()){
                    mpm.deletePlanFrom(mpm.getLastDoneDate());
                    mpm.planFrom(mpm.getLastDoneDate(),mpm.getPlanEndDate(),machine.AVERAGE_METER_PER_DAY);
                    
                    //mpm.rescheduleFrom(mpm.getLastDoneDate(),days_difference,0);
                }
                else{
                    mpm.deleteCompletePlan();
                    mpm.planFrom(mpm.PLAN_START_DATE,mpm.getPlanEndDate(),machine.AVERAGE_METER_PER_DAY);
                    
                    //mpm.rescheduleFrom(mpm.PLAN_START_DATE,days_difference,0);
                }
            }
            else{
                //No problem
        ...
        if(original.cycle_meter != edited.cycle_meter){
            //Cycle meter has been changed;
            if(mpm.isPlanned()){
                int meter_difference = (edited.cycle_meter - original.cycle_meter);
                int days_difference = meter_difference/machine.AVERAGE_METER_PER_DAY;
                
                if(mpm.hasDoneWorkOrders()){
                    mpm.deletePlanFrom(mpm.getLastDoneDate());
                    mpm.planFrom(mpm.getLastDoneDate(),mpm.getPlanEndDate(),machine.AVERAGE_METER_PER_DAY);
                    
                    //mpm.rescheduleFrom(mpm.getLastDoneDate(),days_difference,meter_difference);
                }
                else{
                    mpm.deleteCompletePlan();
                    mpm.planFrom(mpm.PLAN_START_DATE,mpm.getPlanEndDate(),machine.AVERAGE_METER_PER_DAY);
                    
                    //mpm.rescheduleFrom(mpm.PLAN_START_DATE,days_difference,meter_difference);
                }
            }
            else{
                //No problem
        ...
        if(original.starting_meter != edited.starting_meter){
            //Starting meter has been changed;
            if(mpm.isPlanned()){
                if(mpm.hasDoneWorkOrders()){
                    throw new Exception("PM " + mpm.PM_ID + " is in one or more done Jobs. You can not change its' Starting Meter now.");
                }
                else{
                    int meter_difference = (edited.starting_meter - original.starting_meter);
                    int days_difference = meter_difference/machine.AVERAGE_METER_PER_DAY;
        ...
        if(!original.starting_date.equals(edited.starting_date)){
            ...
            if(mpm.isPlanned()){
                if(mpm.hasDoneWorkOrders()){
                    ...
                    int days_difference = Utilities.getDateDifference(edited.starting_date,original.starting_date,connection);
        ...
        if(original.plan != edited.plan){
            //Planing status has been changed;
             if(mpm.isPlanned()){
                if(edited.plan){
                    //FALSE-->TRUE (Plan again)
                    mpm.planFrom(mpm.getLastDoneDate(),mpm.getPlanEndDate(),machine.AVERAGE_METER_PER_DAY);
                }
                else{
                    //TRUE-->FALSE (Stop planing)
                    if(mpm.hasPendingWorkOrders()){
                        throw new Exception("PM " + mpm.PM_ID + " is in one or more pending Jobs. You can not stop planning it without removing them from those Jobs.");    
                    }
                    else if(mpm.hasDoneWorkOrders()){
                        mpm.deletePlanFrom(mpm.getLastDoneDate());
                    }
        ...
        for(int i=0;i<rows;i++){
            ...
            if(basis.equals("Time Period")){
                ...
                if(cycleMeter.isEmpty()){
                    ...
                    messageBar.setMessage("Invalid Cycle Time at row "+(i+1),"ERROR");
                    return false;
                }
