...
public class PlanningAsset{
    ...
    public boolean hasRegistedPMs() throws Exception{
    ...
    public boolean hasRegistedPlanningPMs() throws Exception{
    ...
    public void deletePlan() throws Exception{
        ...
        for(int i=0;i<size;i++){
            ...
            if(mpm.hasPendingWorkOrders()){
                throw new Exception("These are pending jobs for the Machine.");
            }
        }
        for(int i=0;i<size;i++){
            AssetPM mpm = plannedPMs.get(i);
    ...
    public void planOrExtend() throws Exception{
        ...
        for(int i=0;i<size;i++){
            AssetPM mpm = plannedPMs.get(i);
            if(mpm.hasDoneWorkOrders()){
                mpm.planFrom(mpm.getLastDoneDate(),mpm.getPlanEndDate(),this.AVERAGE_METER_PER_DAY);
            }
            else{
                mpm.planFrom(mpm.PLAN_START_DATE,mpm.getPlanEndDate(),this.AVERAGE_METER_PER_DAY);
            }
        }
    }
    ...
    public void reschedule() throws Exception{
        ...
        for(int i=0;i<size;i++){
            ...
            if(mpm.hasDoneWorkOrders()){
                mpm.deletePlanFrom(mpm.getLastDoneDate());
                mpm.planFrom(mpm.getLastDoneDate(),getEndingDate(),AVERAGE_METER_PER_DAY);
            }
            else{
                mpm.deleteCompletePlan();
    ...
    public void extend() throws Exception{
        ...
        for(int i=0;i<size;i++){
            AssetPM mpm = plannedPMs.get(i);
            mpm.planFrom(mpm.getLastScheduledDate(),mpm.getPlanEndDate(),this.AVERAGE_METER_PER_DAY);
        }    
    }
    ...
    public void curtail() throws Exception{
        ...
        for(int i=0;i<size;i++){
            AssetPM mpm = plannedPMs.get(i);
            mpm.deletePlanFrom(mpm.getPlanEndDate());
        }  
    }
